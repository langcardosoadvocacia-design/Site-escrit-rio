...
// --- Declaração Global para o Gtag (Google Ads) ---
declare global {
  interface Window {
    gtag: (...args: any[]) => void;
  }
...
const dispararConversaoWhatsApp = () => {
  if (typeof (window as any).gtag === 'function') {
    ...
    console.log("Conversão Ads Disparada");
  }
};
  ...
  return (
    <nav className={`fixed top-0 w-full z-[100] transition-all duration-300 ${rolou ? 'py-2 bg-white border-b border-black/5 shadow-sm' : 'py-4 md:py-6 bg-transparent'}`}>
      <div className="container mx-auto px-6 md:px-12 flex justify-between items-center">
        <Link to="/" className="transition-transform duration-300 hover:scale-105"><Logotipo reduzido /></Link>
        <div className="hidden lg:flex items-center gap-10">
          <div className="flex items-center gap-8">
            {links.map((link) => (
              <Link key={link.caminho} to={link.caminho} className="group relative py-1">
                <span className={`text-[10px] font-bold uppercase tracking-[0.2em] transition-colors ${local.pathname === link.caminho ? 'text-black' : 'text-black/40 hover:text-black'}`}>{link.nome}</span>
                {local.pathname === link.caminho && <motion.div layoutId="sublinhado" className="absolute bottom-0 left-0 w-full h-[1px] bg-black" />}
              </Link>
            ))}
          </div>
          <a href={LINK_WHATSAPP} target="_blank" rel="noopener noreferrer" onClick={dispararConversaoWhatsApp} className="px-6 py-3 bg-black text-white text-[9px] font-bold uppercase tracking-[0.3em] transition-colors hover:bg-gray-800 cursor-pointer">Atendimento</a>
        </div>
      ...
      <AnimatePresence>
        {menuAberto && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="lg:hidden absolute top-full left-0 w-full bg-white border-b border-black/10 flex flex-col p-8 gap-6 shadow-xl">
            {links.map((link) => <Link key={link.caminho} to={link.caminho} onClick={() => setMenuAberto(false)} className={`text-[12px] font-bold uppercase tracking-[0.2em] py-2 ${local.pathname === link.caminho ? 'text-black' : 'text-black/40'}`}>{link.nome}</Link>)}
            <a href={LINK_WHATSAPP} onClick={dispararConversaoWhatsApp} className="w-full py-4 bg-black text-white text-center text-[10px] font-bold uppercase tracking-[0.3em]">WhatsApp</a>
          </motion.div>
        )}
      </AnimatePresence>
    </nav>
  ...
  <main className="bg-white">
    ...
    <section className="min-h-[85vh] md:min-h-screen flex flex-col justify-center bg-[#fcfcfc] border-b border-black/5">
      <div className="container mx-auto px-6 md:px-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: TRANSICAO_SUAVE }}>
          ...
          {/* TÍTULO SUBSTITUÍDO CONFORME SOLICITADO */}
          ...
          <p className="text-lg md:text-2xl text-black/50 font-light leading-relaxed mb-12 max-w-3xl border-l border-black/10 pl-8">Um escritório de advocacia focado na proteção de ativos e na defesa intransigente da liberdade individual e empresarial.</p>
          <BotaoCTA texto="Solicitar Consulta" />
        </motion.div>
      </div>
    </section>

    {/* Sobre - Foco no Advogado */}
    <section className="py-24 md:py-40 container mx-auto px-6 md:px-12 grid lg:grid-cols-2 gap-20 items-center">
      <div>
        ...
        <p className="text-base md:text-xl text-black/60 font-light leading-relaxed mb-12">O escritório Lang Cardoso Advocacia prioriza o atendimento técnico e personalizado. Não trabalhamos com volume, trabalhamos com soluções estratégicas para casos complexos que exigem atenção absoluta aos detalhes.</p>
      ...
      <div className="relative">
        <img src={IMAGEM_ADVOGADO} alt="Dr. Matheus Lang" className="w-full grayscale shadow-2xl border border-black/5" />
      </div>
    </section>
  ...
  return (
    <main className="bg-white">
      ...
      <section className="py-24 bg-gray-50 border-b border-black/5">
        ...
        <div className="flex justify-center"><BotaoCTA texto="Fale com Matheus Lang" /></div>
      </section>
      <Rodape />
    </main>
  );
...
const PaginaEmpresarial = () => (
  <ModeloPaginaServico area="Direito Corporativo" titulo="Proteção de Ativos." subtitulo="Consultoria estratégica focada na segurança patrimonial e viabilidade econômica do seu negócio."
    dores={[{ t: "Risco Fiscal", d: "Defesa técnica contra autuações e bloqueios administrativos." }, { t: "Gestão de Ativos", d: "Blindagem societária e arquitetura de proteção de bens." }, { t: "Passivos", d: "Reestruturação de dívidas e proteção contra execuções." }, { t: "Contratos", d: "Revisão rigorosa de acordos comerciais de alto valor." }]}
    servicos={[{ t: "Cível Estratégico", d: "Resolução de conflitos de alta complexidade.", i: <Scale size={28} /> }, { t: "Direito Imobiliário", d: "Transações premium e regularização de holdings.", i: <Landmark size={28} /> }, { t: "Tributário", d: "Otimização de carga fiscal e defesas técnicas.", i: <ReceiptText size={28} /> }]}
  />
);
...
const PaginaCriminal = () => (
  <ModeloPaginaServico area="Direito Penal de Elite" titulo="Defesa da Liberdade." subtitulo="Atuação tática de urgência com rigor garantista para proteção dos direitos fundamentais."
    dores={[{ t: "Plantão 24h", d: "Presença imediata em delegacias e audiências de custódia." }, { t: "Tribunal do Júri", d: "Especialista em Plenário com oratória técnica e vigorosa." }, { t: "Crimes Econômicos", d: "Defesa em crimes tributários e contra o sistema financeiro." }, { t: "Prisões", d: "Controle estrito de legalidade para evitar arbitrariedades." }]}
    servicos={[{ t: "Inquéritos", d: "Acompanhamento precoce para evitar abusos de autoridade.", i: <Search size={28} /> }, { t: "Recursos", d: "Atuação tática no STJ e STF para nulidades processuais.", i: <FileText size={28} /> }, { t: "Compliance Penal", d: "Prevenção de riscos para diretores e sócios.", i: <Lock size={28} /> }]}
  />
);
    ...
    <div className="container mx-auto px-6 md:px-12">
      ...
      <div className="grid md:grid-cols-3 gap-12 border-y border-black/10 py-16 mb-24">
        <a href={LINK_MAPA} target="_blank" rel="noopener noreferrer" className="group block">
          ...
          <p className="text-xs text-black/50 leading-loose group-hover:text-black transition-colors">Alameda Montevideo, 322 <br /> Sala 108, Santa Maria - RS <br /> <span className="underline opacity-40">Ver no Google Maps</span></p>
        </a>
        <a href={LINK_WHATSAPP} target="_blank" rel="noopener noreferrer" className="group block" onClick={dispararConversaoWhatsApp}>
          ...
          <p className="text-xs text-black/50 leading-loose group-hover:text-black transition-colors">(55) 3217-6378 <br /> Atendimento direto para urgências <br /> <span className="underline opacity-40">Abrir conversa agora</span></p>
        </a>
        <a href={`mailto:${EMAIL_CONTATO}`} className="group block">
          ...
          <p className="text-xs text-black/50 leading-loose group-hover:text-black transition-colors">{EMAIL_CONTATO} <br /> Consultoria e Documentação <br /> <span className="underline opacity-40">Enviar e-mail</span></p>
        </a>
      </div>
